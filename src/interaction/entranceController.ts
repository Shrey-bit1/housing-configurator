...
export class EntranceController {
  ...
  private edgeAt(clientX: number, clientY: number): { cell: { cx: number; cz: number }; side: Side } | null {
    ...
    if (!cell) return null;
    // Clicked cell must be truly OUTSIDE — empty of ANY occupant under the
    // EFFECTIVE occupancy (a cell claimed by elastic expansion is inside its
    // room, not outside; raw grid alone would wrongly accept it). A stair cell
    // isn't a room/cluster but it isn't "outside" either; allowing it would
  ...
  /** The room/cluster instance id owning (cx,cz) under the EFFECTIVE occupancy
   *  (a grown room's claimed boundary hosts entrances too), if any
   *  (furniture/stairs ignored). */
  private roomClusterOwner(floor: Floor, cx: number, cz: number): string | undefined {
    const id = floor.effectiveOwnerAt(cx, cz);
