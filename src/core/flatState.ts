...
/**
 * The words this file used to declare (run 0042). They live in
 * `src/core/words.ts` now, with every other word a resident reads, and are
 * re-exported here so nothing that already imports them has to move.
 */
import { SEND_IT, GO_TO_GROUP, GROUP_HAS_OLDER, NO_WAY_IN, sendAnyway } from "./words";
export {
  UNTITLED,
  DASH,
  EMPTY_HINT,
...
export function sendButton(phase: FlatPhase, state: SendState, complaints: string[]): SendButton {
  ...
  return { label: sendAnyway(n), notice, awake: true, faults };
}
